;This file is editted on Dec 2014 
pro stacknew
d=80
dv=195
close,/all
...
vm=[vm,vm2]
r=[r,r2]
spawn,'mkdir ~/Dropbox/Research/Supcam/stack383/new2/'
...
spawn,'mkdir /Volumes/MacintoshHD2/supcam/quilt/new2/all'
...
for i=0, n_elements(ra)-1 do begin
  idd=where(n383[x[i]-d:x[i]+d,y[i]-d:y[i]+d] lt -12001)
...
f15=f
for i=0, n_elements(ra)-1 do begin
   aper, im[*,*,i], d, d, flux, eflux,sky,skyerr, 1, apr,[20,30],[-200,48000], /flux, /exact,/silent,setsky=0.0
   f[i,0]=flux[0]
   f[i,1]=eflux[0]
   f15[i,0]=flux[1]
   f15[i,1]=eflux[1]
...
;coordfk5,'/Volumes/MacintoshHD2/supcam/quilt/OBJECT,383.1.fits',[[ra[id]],[dec[id]]],'stackexclude.reg','green','2'
...
openw,1,'/Volumes/MacintoshHD2/supcam/quilt/new2/BVge0.4_err/listUu'
openw,2,'/Volumes/MacintoshHD2/supcam/quilt/new2/BVge0.4_err/listBb'
openw,3,'/Volumes/MacintoshHD2/supcam/quilt/new2/BVge0.4_err/listVv'
openw,4,'/Volumes/MacintoshHD2/supcam/quilt/new2/BVge0.4_err/listNBv'
openw,5,'/Volumes/MacintoshHD2/supcam/quilt/new2/BVge0.4_err/listLCc'
for i=0, n_elements(ra)-1 do begin
...
for i=0,n_elements(ra)-1 do begin
   hextract,n383,h383,newim,newhr,x[i]-d,x[i]+d,y[i]-d,y[i]+d
   writefits,strcompress('~/Dropbox/Research/Supcam/stack383/new2/BVge0.4_err/'+string(i)+'-'+string(ra[i])+string(dec[i])+'dt383.fits',/remove_all),newim,newhr
...
print,' meanratio= ',sum/(sig383/sqrt(n_elements(flux))),' meanratio15= ',sum15/(sig15/sqrt(n_elements(flux15)))
stackim=fltarr(2*d+1,2*d+1)-12000
weight=fltarr(2*d+1,2*d+1)+nuse
for i=0, 2*d do for j=0, 2*d do begin
   id=where(im[i,j,*] lt -200)
   if id[0] ne -1 then begin
   im[i,j,id]=0
...
writefits,'~/Dropbox/Research/Supcam/stack383/new2/BVge0.4_err/stack383err.fits',stackim,newhr
writefits,'~/Dropbox/Research/Supcam/stack383/new2/BVge0.4_err/stack383err.exp',weight,newhr

aper, stackim, d, d, fs, efs,sky,skyerr, 1, apr,[20,30],[-200,48000], /flux, /exact,/silent,setsky=0.0
...
dl=2.89118d10
mr=r-5.0*(alog10(dl)-1)
set_plot,'ps'
device,filename='~/Dropbox/Research/Supcam/stack383/new2/all/Mr383.eps',/color,/encap
plothist,mr,bin=0.2,xtitle=textoidl('M_R'),ytitle='Number',thick=4.5,xthick=3,ythick=3,charsize=1.5,xr=[-23, -18.5],xstyle=1;,yr=[0,50],ystyle=1
